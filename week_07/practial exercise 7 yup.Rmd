...
```{python}
x = np.arange(0, 6, 0.1)
#between 0-6 with a interval of 0.1, so we get 60 values
...
#0 is the mean, because it needs to be normally distributed
#5 is the standard deviation aka sigma
#is the  number of elements you get in array noise aka the number of y's 
...
plt.plot(x, y, 'r-') #plotting the true function
plt.plot(x, y_noise, 'bo') #plotting the generated points
plt.legend(['Generated datapoints', 'Fitted line'])
...
#the predicted values for making the following plot
...
plt.plot(x, y, 'r-') #plotting the true function
plt.plot(x, y_noise, 'bo') #plotting the generated points
plt.plot(x, y_hat, 'g-') #the predicted values
plt.legend(['Generated datapoints', 'Fitted line', 'True function'])
plt.show() #show figure
```
...
X_quadratic = quadratic.fit_transform(x.reshape(-1, 1)) #it's a design matrix

reg = LinearRegression()
reg.fit(X_quadratic, y_noise)

#calculating y_hats
y_quadratic_hat = reg.predict(X_quadratic)

```
```{python}
#plotting the quadratic
#plotting it all 
plt.figure() #new figure
plt.plot(x, y, 'r-') #plotting the true function
plt.plot(x, y_noise, 'bo') #plotting the generated points
plt.plot(x, y_hat, 'g-') #the predicted values
plt.plot(x, y_quadratic_hat, 'm-')
plt.legend(['Generated datapoints', 'Fitted line', 'True function', 'Quadratic function'])
plt.show() #show figure
```

  iii. do a quadratic and a fifth order fit as well and plot them (on top of the plot from 1.2.i)
```{python}
#above is the quadratic fit

#changing degree = 5 to get the fifth order
fifth = PolynomialFeatures(degree=5)
X_fifth = fifth.fit_transform(x.reshape(-1, 1)) #it's a design matrix

reg_fifth = LinearRegression()
reg_fifth.fit(X_fifth, y_noise)

#calculating y_hats
y_fifth_hat = reg_fifth.predict(X_fifth)
```
```{python}
#plotting the quadratic
#plotting it all 
...
plt.plot(x, y_hat, 'g-') #the predicted values
plt.plot(x, y_quadratic_hat, 'm-')
plt.plot(x, y_fifth_hat, 'k-')
plt.legend(['Generated datapoints', 'Fitted line', 'True function', 'Quadratic function', 'Fifth order fit'])
plt.show() #show figure
```
...
```{python}
np.random.seed(10)
new_samples = [y + np.random.normal(0,5,60) for i in range(100)]
```
...
for sample in new_samples:
  new_reg.fit(x, sample)
  predicted_linear_hats.append(new_reg.predict(x))
```
...
for sample in new_samples:
  regressor_quad.fit(X_quadratic, sample)
  predicted_quad_hats.append(regressor_quad.predict(X_quadratic))
```

Fifth order
```{python}
reg_fifth100 = LinearRegression()

predicted_fif_hats = []

for sample in new_samples:
  reg_fifth100.fit(X_fifth, sample)
  predicted_fif_hats.append(reg_fifth100.predict(X_fifth))
```
...
for new_samples in predicted_linear_hats:
  linlines = plt.plot(x, new_samples, 'r-')

for new_samples in predicted_quad_hats:
  qualine = plt.plot(x, new_samples, 'b-')

#highlighting the true value -at first I create a dot in the point (3,9) in the plot, because our function is x^2 ergo the true value of 3 will be at 9 on the y-axis
plt.plot([3], [9], 'ko', markersize = 7)

#naming the lines, but making sure that not all 100 of them gets a name, so only one name will show up
plt.legend([linlines[0], qualine[0]], ['Linear fit', 'Quadratic fit'])

plt.show()
```
Looking at the graphics it looks like the linear fit (red line) is more biased since it consequently lies above the true value of 3, while the quadratic fit lies centered to the true value. 
...
for new_samples in predicted_quad_hats:
  qualines = plt.plot(x, new_samples, 'b-')

for new_samples in predicted_fif_hats:
  #alpha makes the lines a little see through, so we can see the lines better
  fifline = plt.plot(x, new_samples, 'm-', alpha = 0.2)

#highlighting the true value -at first I create a dot in the point (3,9) in the plot, because our function is x^2 ergo the true value of 3 will be at 9 on the y-axis
plt.plot([3], [9], 'ko', markersize = 7)

#naming the lines, but making sure that not all 100 of them gets a name, so only one name will show up
plt.legend([qualines[0], fifline[0]], ['Quadratic fit', 'Fifth order fit'])

plt.show()
```
 Both fits have a low bias, since they both lie centering the true value aka. the black dot. 
