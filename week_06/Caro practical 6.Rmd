---
title: "practical exercise 6 Caro"
...
```{python}
#fitted the best linear regression to the data
...
yhat = X@betas
print(yhat)
#here we see the real values from the data (just to compare)
print(y)
```
...
```{python}
#Next thing that's up is a figure aka. plot
plt.figure()
#telling it which plot it is
plt.scatter(x, y)
plt.plot(yhat, 'g-')
#showing the plot
plt.show()
...
#Making a model Matrix with the length of y and telling it to be three columns long since there are y1, y2 and y3
...
means = np.linalg.inv(np.transpose(X)@X)@np.transpose(X)@y
print(means)
```
...
```{python}
X1 = np.zeros(shape=(len(y), 3))
#making sure that there are 1's in y1, y2 and y3 in the right places, so they can be compared accordingly to the exercise
X1[0:5, 0] = 1
X1[10:15, 0] = 1
X1[0:, 1] = 1
X1[10: , 2] = 1
...
means2 = np.linalg.inv(np.transpose(X1)@X1)@np.transpose(X1)@y
print(means2)
```
